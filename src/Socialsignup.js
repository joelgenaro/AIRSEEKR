...
export default class Socialsignup extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      uncheck: check,
      name:
        this.state.social_data?.social_name?.trim()?.toLowerCase() === 'null'
          ? ''
          : this.state.social_data?.social_name,
  ...
  signup = () => {
    ...
    apifuntion
      ...
      .then(obj => {
        ...
        if (obj.success == 'true') {
          const uservalue = {
            login_type: this.state.social_type,
            social_email: this.state.social_email,
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        ...
        <KeyboardAvoidingView
          style={{ flex: 1, backgroundColor: Colors.theme_color }}
