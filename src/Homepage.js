...
import { RefreshControl, I18nManager, Text, BackHandler, SafeAreaView, StatusBar, KeyboardAvoidingView, Alert, View, StyleSheet, Keyboard, Dimensions, ImageBackground, TouchableOpacity, Image, TextInput, Modal, FlatList, ScrollView } from 'react-native'
    ...
    onOpened = async (openResult) => {
        ...
        try {
            ...
            if (userdata != null) {
                ...
                if (action == 'reject_offer' || action == 'accept_offer' || action == 'accept_offer_user' || action == 'edit_offer' || action == 'send_offer') {
                    this.props.navigation.navigate('Accept_reject_offer', { offer_id: action_id })
                }
            }
            else {
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: Colors.newcolor,  }}>
                ...
                <ScrollView style={{}} ref={(c) => { this.scroll = c }}
                    refreshControl={
                        <RefreshControl
                ...
                >
                    <View style={{ flex: 1, backgroundColor: Colors.newcolor,paddingBottom: windowHeight * 8 / 100, }}>
                        ...
                        <View style={{ paddingHorizontal: windowWidth * 5.5 / 100, marginTop: windowHeight * 2.3 / 100, flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                            <Text style={[styles.txtitem1, { color: Colors.blackColor, }]}>{Lang_chg.adscategory[config.language]}</Text>
                            <TouchableOpacity onPress={() => { this.props.navigation.navigate('Allcategories') }} style={{ justifyContent: 'center', paddingLeft: windowWidth * 2.5 / 100 }}>
                                <Text style={[styles.txtitem11, { color: Colors.red_color, }]}>{Lang_chg.viewall[config.language]}</Text>
                            </TouchableOpacity>

                        </View>

                        <View style={{ justifyContent: 'center', marginTop: windowHeight * 1 / 100, marginHorizontal: windowWidth * 5 / 100,paddingLeft:windowWidth*1/100 }}>
                            {this.state.category_arr != "NA" &&
                                <FlatList
                                    ...
                                    renderItem={({ item, index }) => {
                                        if (index <= 4) {
                                            return (
                                                <View style={{ width: windowWidth * 18.3 / 100, alignItems: 'center', marginTop: windowHeight * 0.5 / 100, marginRight: windowWidth * 4.2 / 100, marginLeft: 2, }}>
                                                    {index < 4 && <TouchableOpacity onPress={() => { this.props.navigation.navigate('Categorywise', { category_name: item.category_name, category_id: item.category_id }) }} style={{ shadowColor: "#000", shadowOffset: { width: 2, height: 2, }, shadowOpacity: 0.20,elevation: 1, width: windowWidth * 18.3 / 100, height: windowHeight * 14 / 100, alignItems: 'center', borderColor: Colors.lightfontcolor, borderWidth: 0, borderRadius: 0, justifyContent: 'center', backgroundColor: Colors.newcolor1, marginBottom: 2 }} >
                                                        <View style={{ width: windowWidth * 10 / 100, height: windowWidth * 11 / 100, alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <View style={{ paddingHorizontal: windowWidth * 5.5 / 100, marginTop: windowHeight * 2 / 100, flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
                            <Text style={[styles.txtitem1, { color: Colors.blackColor, }]}>{Lang_chg.recentAds[config.language]}</Text>
                            <TouchableOpacity onPress={() => { this.props.navigation.navigate('Alllisting') }} style={{ justifyContent: 'center', paddingLeft: windowWidth * 2.5 / 100 }}>
                                <Text style={[styles.txtitem11, { color: Colors.red_color, }]}>{Lang_chg.viewall[config.language]}</Text>
                        ...
                        <View style={{ width: '100%', alignSelf: 'center', paddingBottom: windowHeight * 1 / 100, marginTop: windowHeight * 1 / 100, paddingHorizontal: windowWidth * 4 / 100, }}>
                            {this.state.new_item != "NA" &&
                                <FlatList
                                    ...
                                    renderItem={({ item, index }) => {
                                        return (
                                            <TouchableOpacity activeOpacity={0.7} onPress={() => { this.godetailpage(item, index) }} style={{ width: '46.5%', backgroundColor: Colors.newcolor1, elevation: 2, shadowColor: "#000", shadowOffset: { width: 2, height: 2, }, shadowOpacity: 0.20, alignItems: 'center', alignSelf: 'center', marginHorizontal: '2.5%', marginBottom: windowHeight * 1 / 100, marginTop: windowHeight * 1 / 100, borderRadius: 5, }}>
                                                <View style={{position: 'absolute', paddingHorizontal: 10, paddingVertical: 4,
                                                    backgroundColor: item.for_sale === 0 ? '#100dc9' : '#c90d0d', borderTopLeftRadius: 12, borderBottomLeftRadius: 12, top: 10, right: 0, zIndex: 20
                                                }}>
                                                    <Text style={{fontFamily: Font.Poppins_Bold, color: 'white', fontSize: 10}}>
                                                        {item.for_sale === 0 ? 'LOOKING FOR': 'FOR SALE'}
                                                    </Text>
                                                </View>

                                                <View style={{ borderRadius: 5, backgroundColor: Colors.border_color, width: '100%', height: windowHeight * 17 / 100, backgroundColor: Colors.newcolor1 }}>
                                                    ...
                                                    <ImageBackground source={item.image_arr != 'NA' ? { uri: config.img_url2 + item.image_arr[0].image } : localimag.nopreview} imageStyle={{ resizeMode: 'cover', borderTopLeftRadius: 5, borderTopRightRadius: 5 }} style={{ width: '100%', height: windowHeight * 17 / 100, backgroundColor: Colors.white_light, }} >
                                                ...
                                                <View style={{ marginLeft: windowWidth * 3 / 100, height: windowHeight * 9 / 100, width: '100%', paddingTop: windowHeight * 1 / 100 }}>
                                                    <Text numberOfLines={2} style={styles.txtitem}>{item.ads_title}</Text>
                                                    <Text numberOfLines={1} style={[styles.txtadd]}>{item.address}</Text>
                                                </View>
                ...
                <Footer navigation={this.props.navigation} page={'home'} user_id={this.state.user_id} chatcount={count_inbox} />
